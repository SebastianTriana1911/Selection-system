...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/reclutador/dashboard.css') }}">
    <script src="https://kit.fontawesome.com/10d9a6ff24.js" crossorigin="anonymous"></script>
    <title>Dashboard Reclutador</title>
                    ...
                    @forelse($reclutadores as $reclutador)
                        <article class="contenedor-instructor">
                            ...
                            <article class="contenedor-nombre">
                                <h1 class="titulo">{{ $reclutador->user->nombre }} {{ $reclutador->user->apellido }}
                                </h1>
                            </article>

                            <article class="contenedor-opciones">
                                <form class="formulario"
                                    action="{{ route('user.destroy', ['id' => $reclutador->user_id]) }}"
                                    method="POST">
                                    @csrf
                                ...
                                <article class="rol">
                                    <form class="formulario-2"
                                        action="{{ route('update.rol', ['id' => $reclutador->user_id]) }}"
                                        method="POST">
                                        @csrf
